...
// }
// 创建一个 axios 实例
console.log(store.state.d2admin.api.base)
const service = axios.create({
  headers: {
    // Authorization: utils.cookies.get('token'),
    'Content-Type': get(utils.cookies.get('token'), 'headers.Content-Type', 'application/json')
  },
  timeout: 5000,
...
service.interceptors.request.use(config => {
  config.baseURL = store.state.d2admin.api.base
  config.headers.Authorization = utils.cookies.get('token')
  return config
}, error => {
