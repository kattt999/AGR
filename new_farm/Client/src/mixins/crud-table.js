...
export default {
  ...
  computed: {
    ...
    vNodeSearchForm() {
      const node =
        <el-form {...{ attrs: this.search.form }} class="is-thin">
          ...
          <el-form-item label={this.$t('operate')}>
    ...
    settingActions() {
      let _size = 12
      if (store.state.d2admin.size.value === 'medium' || store.state.d2admin.size.value === 'default') {
        _size = 40
      } else if (store.state.d2admin.size.value === 'small') {
        _size = 28
      }
      ...
      const width = configStatic.length > 0 ? configStatic.reduce((result, item) => {
        if (item.icon) result += _size
        if (item.label) result += item.label.length * _size
