...
module.exports = {
  index: async function (req, res) {
    ...
    Device.count(_qdata.query).exec(function (cerr, _cdata) {
      if (cerr) {
      ...
      sails.log(_qdata)
      Device.find(_qdata.query).sort(_qdata.sort).skip(_qdata.skip).limit(_qdata.limit).exec(function (err, _data) {
        if (err) {
  ...
  query: function (req, res) {
    let qVo = req.body
    Device.findOne({ id: qVo.id }).exec(function (err, _data) {
      if (err) {
  ...
  update: function (req, res) {
    ...
    delete _qVo.id
    Device.update({ id: qVo.id }, _qVo).exec(function (err, _data) {
      if (err) {
        sails.log(err)
        log.appError(JSON.stringify(err), req);
        return res.json({
  ...
  create: function (req, res) {
    ...
    Device.create(qVo).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  delete: function (req, res) {
    let _vo = req.body
    Device.destroy(_vo).exec(function (err, vo) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
