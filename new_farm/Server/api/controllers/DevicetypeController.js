...
module.exports = {
  index: function (req, res) {  
    ...
    DeviceType.find(_qdata.query).sort(_qdata.sort).exec(function (err, _data) {
      if (err) {
  ...
  query: function (req, res) {
    let qVo = req.body
    DeviceType.findOne({ id: qVo.id }).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  update: function (req, res) {
    ...
    delete _qVo.id
    DeviceType.update({ id: qVo.id }, _qVo).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  create: function (req, res) {
    ...
    DeviceType.create(qVo).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  delete: function (req, res) {
    let _vo = req.body
    DeviceType.destroy(_vo).exec(function (err, vo) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
