...
module.exports = {
  index: async function (req, res) {
    ...
    if(_qdata.query.created_at  != undefined){
      _qdata.query.createdate = _qdata.query.created_at 
      delete _qdata.query.created_at 
    }
    sails.log(_qdata)
    Producter.count(_qdata.query).exec(function (cerr, _cdata) {
      if (cerr) {
      ...
      sails.log(_qdata)
      Producter.find(_qdata.query).sort(_qdata.sort).skip(_qdata.skip).limit(_qdata.limit).exec(function (err, _data) {
        if (err) {
  ...
  query: function (req, res) {
    let qVo = req.body
    Producter.findOne({ id: qVo.id }).exec(function (err, _data) {
      if (err) {
  ...
  update: function (req, res) {
    ...
    delete _qVo.createdate
    delete _qVo.id
    Producter.update({ id: qVo.id }, _qVo).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  create: function (req, res) {
    ...
    Producter.create(qVo).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  delete: function (req, res) {
    let _vo = req.body
    Producter.destroy(_vo).exec(function (err, vo) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
