...
module.exports = {
  index: async function (req, res) {
    ...
    sails.log(_qdata)
    Farmproducts.count(_qdata.query).exec(function (cerr, _cdata) {
      if (cerr) {
      ...
      Farmproducts.find(_qdata.query).sort(_qdata.sort).skip(_qdata.skip).limit(_qdata.limit).exec(function (err, _data) {
        if (err) {
  ...
  query: function (req, res) {
    let qVo = req.body
    Farmproducts.findOne({ id: qVo.id }).exec(function (err, _data) {
      if (err) {
  ...
  update: function (req, res) {
    ...
    delete _qVo.id
    Farmproducts.update({ id: qVo.id }, _qVo).exec(function (err, _data) {
      if (err) {
        log.appError(JSON.stringify(err), req);
        return res.json({
          code: 1,
  ...
  create: function (req, res) {
    ...
    Farmproducts.create(qVo).exec(function (err, _data) {
      if (err) {
        sails.log(err)
        log.appError(JSON.stringify(err), req);
        return res.json({
