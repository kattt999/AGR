...
export default context => {
  ...
  function getMenus(menuSource) {
    ...
    function maker(menus, sourceItem) {
      ...
      if (_ff === 'zh-chs') {
        ...
        menu.title = sourceItem.menu_name_en
      }
      // menu.title = sourceItem.menu_name
  ...
  function getRoutes(menuSource) {
    ...
    function isEffectiveRoute(sourceItem) {
      ...
      const hasAllRequiredProperties = _data.reduce((res, keyname) => !((!res || sourceItem[keyname] === '')), true)
    ...
    function maker(routes, sourceItem) {
      if (hasRouteChildren(sourceItem)) {
        ...
        let _route_cache = false;
        if (sourceItem.route_cache != undefined && sourceItem.route_cache == 1) {
          _route_cache = true
        }
        ...
        let _ff = utils.cookies.get('lang') || env.VUE_APP_I18N_LOCALE
        if (_ff != 'zh-chs') {
          route.meta.title = sourceItem.menu_name_en
        }
  ...
  return {
    ...
    actions: {
      ...
      async load({ state, rootState, commit, dispatch }, { focus = false, to = '', data }) {
        ...
        // // [ 路由 ] 计算路由
        // console.log(JSON.stringify(getRoutes(source)))
        // console.log(JSON.stringify(routesOutLayout))
